    ...
    switch (hdr->type) {
        case ICMP_TYPE_ECHO:
            ...
            icmp_output(ICMP_TYPE_ECHOREPLY, hdr->code, hdr->values,
                        data + ICMP_HDR_SIZE, len - ICMP_HDR_SIZE, iface->unicast, src);
            break;
        default:
            /* ignore */
            break;
...
int icmp_output(uint8_t type, uint8_t code, uint32_t values,
    ...
    hdr->type = type;
    hdr->code = code;
    hdr->sum = 0;  // Clear checksum field before calculation
    hdr->values = hton32(values);
    msg_len = ICMP_HDR_SIZE + len;
    // Copy the payload data
    if (len > 0) {
        if (len > ICMP_BUFSIZ - ICMP_HDR_SIZE) {
            errorf("too long, len=%zu > %zu", len, ICMP_BUFSIZ - ICMP_HDR_SIZE);
            return -1;
        }
