...
static void arp_cache_delete(struct arp_cache *cache) {
    ...
    memset(&cache->pa, 0, sizeof(cache->pa));
...
static int arp_reply(struct net_iface *iface, const uint8_t *tha, ip_addr_t tpa,
    ...
    // Set sender protocol address (our IP address)
    reply.spa[0] = ((struct ip_iface *)iface)->unicast & 0xff;
    reply.spa[1] = ((struct ip_iface *)iface)->unicast >> 8 & 0xff;
    reply.spa[2] = ((struct ip_iface *)iface)->unicast >> 16 & 0xff;
    reply.spa[3] = ((struct ip_iface *)iface)->unicast >> 24 & 0xff;
    // Set sender hardware address (our MAC address)
    memcpy(reply.sha, NET_IFACE(iface)->dev->addr, ETHER_ADDR_LEN);

    // Set target protocol address (requester's IP address)
    reply.tpa[0] = tpa & 0xff;
    reply.tpa[1] = tpa >> 8 & 0xff;
    reply.tpa[2] = tpa >> 16 & 0xff;
    reply.tpa[3] = tpa >> 24 & 0xff;
    // Set target hardware address (requester's MAC address)
